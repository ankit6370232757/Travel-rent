...
const Button = styled(motion.button)`
  ...
  background: ${({ disabled }) => disabled ? "#333" : "#3ea6ff"};
...
const UploadBox = styled.label`
  ...
  margin-bottom: 25px; min-height: 140px;
  input { display: none; }
...
export default function Withdraw() {
  ...
  const saveAccount = async () => {
    ...
    setLoading(true);
    try {
      // ✅ Saves to /api/wallet/withdrawal-accounts
  ...
  const submitWithdraw = async () => {
    ...
    // Validations
    if (!numAmount || numAmount < 300) return setStatus({ type: "error", msg: "Minimum withdrawal is $300" });
    if (!selectedAccount) return setStatus({ type: "error", msg: "Select a withdrawal account" });
    if (numAmount > balance) return setStatus({ type: "error", msg: "Insufficient Balance" });

    setLoading(true);
    try {
      const acc = accounts.find(a => a.id === Number(selectedAccount));
      
      // ✅ POST to /api/wallet/withdraw
      // Passes separate methodName and address as expected by updated service
      await api.post("/wallet/withdraw", { 
        amount: numAmount, 
        methodName: acc.method_name, 
        address: acc.address 
      });
        ...
        {view === "withdraw" ? (
          <motion.div key="withdraw" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <InfoBox>
              ...
              <div><span>Min. Withdrawal</span><span>$300.00</span></div>
            </InfoBox>
            ...
            {accounts.length === 0 ? (
              <div style={{textAlign:'center', padding:'30px', background:'rgba(255,255,255,0.03)', borderRadius:12, marginBottom:20, border:'1px dashed #444'}}>
                ...
                <Button onClick={() => setView("add")} style={{width:'auto', padding:'12px 25px'}}>+ Add Method</Button>
              </div>
            ) : (
              <>
                ...
                <ToggleLink onClick={() => setView("add")}><Plus size={14}/> Add New Account</ToggleLink>
              </>
            )}
          ...
          <motion.div key="add" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <div style={{marginBottom: 20, color:'#aaa', fontSize:14}}>
               Choose an admin-approved method.
            </div>
            ...
            <UploadBox>
              {newAccount.qrCode ? (
                <>
                  ...
                  <span style={{color: '#ff4d4d', cursor:'pointer'}} onClick={(e)=>{e.preventDefault(); setNewAccount({...newAccount, qrCode: ""})}}>
                    <X size={14}/> Remove
                  </span>
                </>
              ) : (
                <> <Upload size={24} /> <span>Upload QR Code (Optional)</span> </>
              )}
            ...
            <div style={{marginTop: 20, textAlign: 'center'}}>
               <span style={{color: '#888', cursor: 'pointer'}} onClick={() => setView("withdraw")}>
